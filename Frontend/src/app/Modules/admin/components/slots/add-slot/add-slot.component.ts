...
export class AddSlotComponent implements OnInit,OnDestroy {
  ...
  slotForm = this.fb.group({
    ...
    therapyCategory:['',Validators.required],
    slotName:[''],
    onCall: [false]
  });
  ...
  displayedColumns: string[] = [ 'roomName','slotName','startTime', 'endTime','therapyCategory','weekDay','slotStatus', 'manage'];
  ...
  ngOnInit(): void {
    ...
    this.roomSubscription = this.getRoom()
  ...
  roomName : any;
  findRoomName(id){
    this.roomName = this.rooms.find(x=>x._id==id).roomName
  }
  ...
  onSubmit(){
    ...
    let data ={
      ...
      slotName: this.weekdayabbreviation+'-'+ this.roomName +'-'+this.slotForm.get(['startTime']).value,
  ...
  editSlot(id){
    ...
    this.roomName = this.rooms.find(x=>x._id==rName).roomName
