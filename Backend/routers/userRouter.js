...
router.post('/adduser', authToken, async (req,res)=>{
     ...
     //lets validate data before we make a user
        ...
        const { name, email, password, role, md, qualifications, experiences, qualification_id, qualification_file,
            idProof_id, idProof_file, experience_id, experience_file, birthDate, joiningDate } = req.body
    ...
    //lets validate data before we make a user
        ...
        const { name, email, password, role, therapyCategory, md, qualifications, experiences, qualification_id, qualification_file,
            idProof_id, idProof_file, experience_id, experience_file, birthDate, joiningDate, sharePercentage } = req.body
...
router.get('/user', authToken, async(req,res)=>{
    try {
        const user = await User.find({}).sort({_id: -1}).populate('therapyCategory')
        res.send(user)       
    } catch (error) {
        res.send(error)        
    }
...
router.get('/gettherapist', authToken, async(req,res)=>{
    try {       
        const user1 = await User.find({role:"therapist"}).populate('therapyCategory')
        res.send(user1)       
    } catch (error) {
        res.send(error)
    }
...
router.get('/getadmin', authToken, async(req,res)=>{
    try {      
        const user1 = await User.find({role:"admin"})
        res.send(user1)        
    } catch (error) {
        res.send(error)
    }
...
router.patch('/user/:id/:celebid', authToken, async(req,res)=>{
    ...
    const { name, email, password, role, therapyCategory, md, qualifications, experiences, sharePercentage, 
        birthDate, joiningDate} = req.body
